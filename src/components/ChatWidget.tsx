...
export function ChatWidget() {
  ...
  async function sendText(text: string) {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: data.text ?? 'Sorry, something went wrong.', time: now() }])
    } catch {
      setMessages(prev => [...prev, { role: 'model', text: "Sorry, I'm having trouble connecting. Please try again.", time: now() }])
    } finally {
      setLoading(false)
    }
  }
    ...
    <>
      {open && (
        ...
        >
          ...
          {!minimized && <div className="shrink-0 px-3 py-3 bg-white border-t border-gray-100 flex gap-2">
            <input
      ...
      <button
        onClick={() => {
          if (open) {
            setOpen(false)
            setMinimized(false)
            setMessages([{ role: 'model', text: "Hi! I'm FIPL's virtual assistant. How can I help you today?", time: now() }])
          } else {
            setOpen(true)
          }
        }}
